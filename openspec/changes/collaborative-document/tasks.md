...
- [x] 37.1 Replace the stub in `src/agent/search-web.js` with a real Tavily call per the pinned signature: `POST TAVILY_URL`, `Authorization: Bearer <TAVILY_API_KEY>`, body `{ query, max_results: SEARCH_MAX_RESULTS, search_depth: 'basic', include_answer: false }`, `SEARCH_TIMEOUT_MS` timeout, `opts.signal` honoured (design D33) — **done and verified live against the real Tavily endpoint** (with a fake key, so a real `401` came back rather than a mock) — no `TAVILY_API_KEY` was available to prove a real `200`; that's Gate B (39.1)
- [x] 37.2 Shape results to `{ title, url, content }`, truncating `content` at the last word boundary before `SEARCH_SNIPPET_CHARS` with a trailing `…` (design D37) — **done, verified against a local mock Tavily server:** a 1000-char result truncated to ≤501 chars ending in `…` with no broken word or double space; a 4th result was dropped (capped at `SEARCH_MAX_RESULTS`); a short result passed through unchanged
- [x] 37.3 Return `{ available: false, message }` for: no key, `429`/`432`/`433`, any other HTTP error, a timeout, an aborted signal, or a malformed body — never throw (design D38) — **done, verified directly:** unset key, empty query, a real `401` from Tavily, and a pre-aborted signal all returned `{ available: false, message }` without throwing; the timeout path shares the same `AbortSignal.any`/`err.name` mechanism, confirmed separately against a real `fetch` (`err.name === 'TimeoutError'`) but not re-proven end-to-end through `searchWeb()` itself
- [x] 37.4 Delete the duplicate `search_web` schema in `llm-client.js` and the inline stub response in `orchestrator.js`; both now import from `search-web.js` (design D34). Grep for `search_web` afterwards and confirm exactly one schema and one handler exist — **done:** `grep -rn "SEARCH_WEB_SCHEMA"` shows one definition (`search-web.js`) and one import site (`llm-client.js`); `dispatchTool` in `orchestrator.js` is the only call site of `searchWeb()`
- [x] 37.5 Log one startup warning when `TAVILY_API_KEY` is unset, naming the variable, and never log the key or a full search response body — **done, verified live:** agent boot log shows `WARNING: TAVILY_API_KEY is not set...`; no code path logs `process.env.TAVILY_API_KEY` or a response body

## 38. Rumaisa — `append_doc`, the search flow and sourcing

- [x] 38.1 Add `appendDoc(doc, text, opts)` to `src/agent/doc-client.js` using `typeIntoNewParagraph()`, honouring `opts.isCancelled`, and register the `append_doc` tool schema (design D36) — **done, verified directly against a real `Y.Doc`:** a normal append lands as a new paragraph with the text (URL included) intact; empty/whitespace-only text returns `{ ok: false }`; cancelling mid-append stops within one chunk (`"this i"` of a longer sentence), leaving the prefix and no corruption — same proof shape as Milestone D's typing-cancellation test
- [x] 38.2 Dispatch `append_doc` in the orchestrator like `edit_doc`, counting it as a document change so the turn can end successfully — **done:** `dispatchTool` returns `editedDocument: Boolean(result.ok)` for `append_doc` exactly as it does for `edit_doc`; not yet exercised through a real model turn (needs `GROQ_API_KEY`, Gate B 39.5/39.6)
- [x] 38.3 Publish `SEARCH_ACK` when a `search_web` call is dispatched and nothing has been published for this turn yet, so the acknowledgement always precedes the search (design D35) — **done:** a per-turn `publishedThisTurn` flag, set by every `publishReply` call (the model's own words included), gates the fallback `SEARCH_ACK` publish before a `search_web` dispatch. Traced by hand, not yet exercised through a real model turn — Gate B 39.4 proves the ordering with real log timestamps
- [x] 38.4 Cap searches at `SEARCH_MAX_PER_TURN`; a further call returns `{ available: false, message: 'Search limit reached for this turn.' }` (design D37) — **done:** a per-turn `searchCallCount`, checked and incremented before each `search_web` dispatch across all retry attempts, not just within one
- [x] 38.5 Pass the turn's `AbortSignal` into `searchWeb()` so a barge-in drops an in-flight search (design D28, D33) — **done, verified directly:** `dispatchTool` passes `signal: opts.signal` through to `searchWeb()`; a pre-aborted signal reproduces the exact cancellation path a barge-in would trigger (37.3's test)
- [x] 38.6 System prompt: state the sourcing rule (any fact written from a search carries a URL from that search's results, never an invented one), the split between `edit_doc` (replace existing text) and `append_doc` (add something new), and that the spoken reply is one sentence while the written text is the fuller version — **done:** `SYSTEM_PROMPT` in `llm-client.js` states all three; not yet measured against a real model's actual behaviour (Gate B's two open questions, 39.2/39.5)
